...
- *The accuracy of a given model*. The more accurate the model the fewer costs (false postives) and the more benefits (true positives), leading to higher overall profit.
...
## Expected Benefit Framework

The expected benefit framework, while important, is unfortunately somewhat obscure and difficult to understand (especially if you are not comfortable with mathematical notation).  The first thing to recognize is that the framework is more a conceptual tool  for  refining the business problem than it is a tool for, say, calculating profit. So don't worry about figuring out how to translate this into code. 

The expected benefit of targeting is:

$$EB_T(x) = p(Stay|x,T)*(u_{stay}(x)-c)+[1-p(Stay|x,T)]*(u_{NotStay}(x)-c)$$
...
2.  The probability she will  leave *if targeted* multiplied by the profit if she  leaves (minus the incentive). ($1-p(Stay|x,T)$ is the same as $p(Leave|x,T)$.)

Notice that the probability of staying is conditional on being targeted---$p(Stay|x,T)$---yet that is information we do not have (and might be difficult to estimate). We used  the tree model  to estimate $p(Leave)$.   But this formula  requires  something different---an estimate of the probability of customer will respond to the special offer, will stay if targeted.  Who knows how likely that is. To estimate that probability we would need, for example, evidence from prior marketing campaigns. The unavailability of this data in the present case underscores the point that this is a formulation designed to help us think about the business problem, not to be used in calculating profit.  

Next, we examine the expected benefit of *not* targeting:

$$EB_{notT}(x) = p(Stay|x,notT)*(u_{stay}(x)-c)+[1-p(Stay|x,notT)]*(u_{NotStay}(x)-c)$$

This has a similar interpretation to the equation above for $EB_T$. 

The point of these complicated formulations is to better understand what we are trying to accomplish with the marketing campaign and how we should accomplish it---answering the question, in particular, of which customers should get the special offer. Intuitively, these will be customers for whom the value of targeting (VT), $EB_T(x) - EB_{notT}(x)$, is largest. These are customers whose probability of staying will be most impacted by the special offer and whom it will be most profitable to target: rather than leaving they stay and continue paying fees. (By contrast, a customer who was already going to stay, even without the offer, will not be profitable to target, nor will it be profitable to target a customer who would not stay, even with the offer.) DSB expresses this result as $VT=\Delta(p)∗u_{Stay}(x)−c$, the change in the probability of staying multiplied by revenue associated with staying minus the cost of the incentive.  Basically $\Delta(p)$ serves as a weight on profitability. Customers with low $\Delta(p)$ will tend to have low profitability and, conversely, customers with high $\Delta(p)$ will tend to have high profitability.
