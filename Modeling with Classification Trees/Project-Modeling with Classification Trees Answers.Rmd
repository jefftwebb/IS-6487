...
```{r}
(first_tree_model <- rpart(answered~income, data = d))
rpart.plot(first_tree_model)

sum(predict(first_tree_model, type = "class")==d$answered)/nrow(d)
```

### 3.  What is the entropy in the tree's first terminal node, 2) income>=63270?

```{r}
-.7238*log(.7238) - .2762*log(.2762) 
...
```{r}
sum(predict(tree_model, type = "class")== d$answered)/nrow(d) #.7642
```
